...
The plgg monorepo places all publishable packages under `packages/` with a flat structure: `packages/plgg/`, `packages/plgg-foundry/`, `packages/plgg-kit/`, and `packages/example/`. Each package has its own `tsconfig.json`, `vite.config.ts`, `package.json`, and `CHANGELOG.md`. TypeScript path aliases are declared per-package in `tsconfig.json`, mapping the package name to its `./src/*` directory so that cross-package imports resolve correctly during development. The root `scripts/` directory contains shared shell scripts (`scripts/tsc-plgg.sh`, `scripts/test-plgg.sh`) that operate across all packages.
