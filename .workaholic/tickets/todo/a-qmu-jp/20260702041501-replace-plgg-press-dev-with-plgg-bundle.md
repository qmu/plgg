...
plggmatic (strip dev): `src/Dev/usecase/dev.ts` (+ spec) DELETE, `src/index.ts` barrel drop the `dev`/dev-helper exports. Reconcile 213411 (see note appended to it).
...
4. Delete `plggmatic/src/Dev/usecase/dev.ts` (+ spec) and its barrel export; append the supersede note to 213411.
...
6. `scripts/tsc-plgg.sh` + `scripts/test-plgg.sh` green ≥90%; verify prod build output unchanged.
...
**Verification method:** `scripts/tsc-plgg.sh` exits 0; `scripts/test-plgg.sh` green ≥90%; guide build diff (pre vs post) empty; edit-a-theme-file → browser refresh demonstrated on the running guide (light/dark), no restart; `git grep -n "plgg-press/dev\b\|Dev/usecase/dev" packages` shows the old servers removed.
