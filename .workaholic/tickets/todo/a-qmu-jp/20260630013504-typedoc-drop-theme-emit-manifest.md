...
Remove typedoc-vitepress-theme from the typedoc plugin list (keeping typedoc-plugin-markdown), flatten output (parametersFormat/indexFormat → list) toward the shape plgg-md handles best, and rewire gen-api.mjs so its emitted per-package API sidebar manifest is consumed by the new plgg-native site config rather than VitePress. Normalize any out-of-subset construct the spike flagged. This is the keep-typedoc/drop-theme decision made concrete; the render-verification of the output is performed downstream in the build ticket (which has renderMarkdown available).
...
4. Confirm api/ output and the manifest remain gitignored as before; do NOT remove vitepress itself here (ticket 14).
...
- The plgg-md render-verification of regenerated pages is intentionally deferred to ticket 10 (which has renderMarkdown), so this ticket depends only on the spike.
