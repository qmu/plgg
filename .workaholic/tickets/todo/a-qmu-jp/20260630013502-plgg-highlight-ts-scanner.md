...
commit_hash:
category:
depends_on: [20260630013458-md-corpus-spike-and-decisions.md, 20260630013501-plgg-md-inline-fold-to-html.md]
---
...
- `/home/ec2-user/projects/plgg/packages/plgg-bundle/src/vendors/transpiler.ts` - proof that `import ts from 'typescript'` exposes ts.ModuleKind/ScriptTarget; same default export gives createScanner/SyntaxKind, no cast
...
1. Create packages/plgg-highlight (package.json mirroring plgg-view): dependencies plgg, plgg-view, plgg-md via file:; devDependencies include typescript ^6.0.3 (reusing the existing version — a build-time tool, NOT a runtime dependency).
...
5. Implement highlightCss(): Styles mapping each TokenKind to a plgg-view color token, for the theme to merge into the page stylesheet.
...
- Like plgg-bundle, this package imports typescript from its own node_modules (clean-runner masking) — it must be installed wherever the guide builds (ticket 15).
