...
Precursor enhancement that closes the verified builder gap: plgg-view currently exposes typed builders ONLY for div, section, header, main_, form, li, a, span, strong, em, label, button, h1, h2, p, ul, input — and el() returns a string-branded Html<Msg> that is NOT assignable to the narrow Flow/Phrasing/ListItem unions, so el()-built nodes cannot nest inside typed containers. Add typed builders and content-model wiring for the tags the Markdown renderer, highlighter, and theme need: h3-h6, ol, code, pre, img, br, hr, blockquote, the table family (table/thead/tbody/tr/td/th), and details/summary. Breaking/additive changes are fine (plgg is its own only consumer). SSR-only tags need no client runtime changes.

**Proof of value:** plgg-test specs: renderToString over trees using the new builders (h3, ol/li, blockquote, pre>code, img, hr, a full table thead/tbody/tr/th/td, details>summary) emits the expected HTML — green under scripts/test-plgg-view.sh.
...
- `workaholic:implementation` / `policies/coding-standards.md` — strict no-as/any/ts-ignore, printWidth 50; pin tag literals via ElementContent like existing builders
...
- `/home/ec2-user/projects/plgg/packages/plgg-view/src/Html/usecase/renderToString.ts` - verify isSafeTag allows the new tags and they serialize correctly (escape stays the chokepoint)
...
1. Add phrasing-level heading builders h3,h4,h5,h6 (mirroring h1/h2 via phrasingEl) and extend Flow to include them.
2. Add ol as a listEl (li children), code and pre (define a content model: code accepts Phrasing incl. text; pre accepts a code child or text — model per spike findings), img and br as voidEl, hr as voidEl, blockquote as flowEl.
...
4. Add details (flow container) and summary (phrasing container) builders for the CSS-only sidebar; ensure details accepts a summary child plus Flow.
5. Update isSafeTag's allowlist (if grammar-based, confirm new tags pass; otherwise add them) and confirm renderToString emits/escapes them safely.
