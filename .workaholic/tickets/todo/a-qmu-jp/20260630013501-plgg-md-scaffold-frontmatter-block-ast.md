...
1. Create packages/plgg-md with package.json (name plgg-md, type module, build: plgg-bundle, test: tsc --noEmit && plgg-test src), bundle.config.ts, tsconfig.json, .prettierrc.json (printWidth 50), src/index.ts barrel; dependencies plgg + plgg-view via file:, devDeps plgg-bundle/plgg-test/typescript/@types/node mirroring plgg-view.
...
- Frontmatter is layout-marker only — the home hero/features data lives in the plgg-press theme (ticket 8), resolving the nested-YAML mismatch.
- Rendering and inline parsing are deferred to ticket 5 to keep this commit reviewable.
