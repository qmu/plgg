---
created_at: 2026-07-28T09:04:00+09:00
author: a@qmu.jp
type: enhancement
layer: [UX]
effort: 2h
commit_hash:
category: Changed
depends_on: [20260728090300-highlight-span-exactly-once.md]
mission: make-the-column-strip-a-real-navigation-surface
---

# Strip 5/11 — Back means "close that column", including several steps of it

## Overview

A navigation runtime that pushes history without handling
`popstate` is worse than no runtime: Back leaves the reader
on a URL whose composition no longer matches what the DOM
shows. This ticket makes Back exact — it removes the column
the matching forward navigation added and restores the
previous composition, and it does so repeatedly, all the way
back to the entry page.

## Key files

- `packages/plggmatic/src/Navigate/usecase/navigate.ts` —
  the pure reconciler belongs here.
- `packages/plggmatic/src/Navigate/usecase/navigationScript.ts`
  — the `popstate` listener and the DOM application.
- `packages/plggmatic/src/Navigate/usecase/compositionUrl.ts`
  — decoding the composition the browser restored.

## Approach

- **Reconcile, do not undo.** The listener does not remember
  what each navigation did; it decodes the composition the
  restored URL carries and reconciles the strip to it. A
  pure `reconcile(current, target)` returns the ordered plan
  — which existing columns to keep (matched by route and
  span, in order), which to drop, which to fetch — and the
  runtime executes it. That makes Back, Forward, a
  multi-step Back, and a pasted URL the same operation, and
  it is the shape ticket 10's assistant needs anyway.
- Kept columns keep their DOM nodes, so going Back and
  Forward across a column does not reset its scroll or
  refetch it.
- The very first entry has no pushed state: reaching the
  entry composition by Back must restore exactly the strip
  the server would render for that URL.
- The reconciler is a pure function over data — the whole
  correctness argument is unit-testable, and the browser run
  confirms the wiring rather than discovering the logic.

## Quality Gate

- **Acceptance:** in a real browser on port 4130 — open
  three columns by clicking two links, press Back twice, and
  observe after each: the rightmost column is gone, the
  remaining columns are the SAME DOM nodes (a marker set on
  a kept node survives), scroll positions are intact, and
  the URL matches the composition shown. Press Forward and
  the column returns. Screenshot recorded.
- `reconcile` spec covers: append, drop, drop-many, replace
  a middle column, span-only change, and identical
  compositions (a no-op plan).
- Back to the entry page restores the server's own strip,
  verified live.
- `scripts/tsc-plgg.sh` clean; `scripts/test-plgg.sh` green;
  >90% coverage across all four metrics; no `as`/`any`/
  `ts-ignore`; Prettier `printWidth: 50`.

## Policies

- `workaholic:operation` — the running surface recovers: any
  history entry, however reached, is reconstructible from
  its URL alone.

## Final Report

Development completed as planned. Back closes the column
the matching navigation opened, repeatedly, and Forward
brings it back.

**Reconcile, don't undo.** The listener does not remember
what each navigation did. It reads what the restored URL
says (`target()`), reads what is on screen (`current()`),
and makes the second equal the first. Back, Forward, a
multi-step Back and a pasted URL are then one operation.

The comparison is by VALUE in the URL's own alphabet: a
column carries its route AND, when it has one, its passage
(`data-pm-span`, new in this ticket), so a column at the
right route showing the wrong passage counts as different
and is refetched. Columns in the common prefix keep their
DOM nodes, so crossing them costs no fetch and no scroll.

A restored composition is fetched with its passage in the
URL (`route?q=…`), so the SERVER locates and marks it —
this runtime never paints a highlight, which is what keeps
the exactly-once guarantee whole across a popstate.

When the head route itself differs, the reconciler reloads:
the URL is already correct and the nav columns belong to a
different page, so rendering it is the honest move rather
than stitching a half-right strip.

Verified live in a real browser on port 4130:

```
open, open        /concepts/ | /getting-started | /packages/plgg/
                  url ?c=/getting-started,/packages/plgg/
Back              /concepts/ | /getting-started
                  head node still the SAME node, scrollTop still 260
                  second column still the SAME node
Back              /concepts/          url /concepts/
Forward           /concepts/ | /getting-started
window sentinel set before any of it: alive throughout

open("/concepts/option", "Option, not null")
  url    /concepts/?c=/concepts/option:Option~c%20not%20null
  column carries data-pm-span="Option, not null"
  mark   "Option, not null"           (server-located)
  current() === target()              (the reconciler's invariant)
Back → 1 column, 0 marks

Back across a DIFFERENT head route → the right page renders
```

Screenshot: `strip-t5-popstate.png`.

### Discovered Insights

- **Insight**: comparing columns by route alone would have
  been wrong.
  **Context**: two history entries can name the same route
  with different passages. Putting the passage on the
  element and comparing the whole entry makes the
  reconciler's notion of "the same column" identical to the
  URL's.
- **Insight**: a column is fetched from `route?q=<span>`,
  not from the bare route.
  **Context**: the mark has to come from the server, or the
  exactly-once guarantee would have a client-side hole. It
  also fixed a latent gap from ticket 2, where an
  assistant-opened column with a passage would have shown
  no mark at all.
