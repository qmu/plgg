...
**Audit** (all 25 packages, 2026-07-05 / plgg-ui added 2026-07-09 / plgg-cms split from plggpress 2026-07-09; 16 conformant, 7 exempted, 2 content apps):
...
| `plgg-ui` | legacy Feature/ | conformant — no third-party imports (the UI engine re-homed from plggmatic, trip plggmatic-extraction-cut; depends on `plgg` + `plgg-view` only, imports neither `plggpress` nor `plggmatic` — dependency direction one-way; consumed by `plggpress` (theme, since A2), `plgg-cms` (Admin), and `plggmatic` (facade)) |
...
| `plgg-cms` | legacy Feature/ | **EXEMPT** — the dynamic content surface split from plggpress (2026-07-09); serves (`node:http`) + persists (`node:fs`, plgg-sql) outside a `vendors/` boundary; depends on `plggpress` one-way (never the reverse) |
