...
1. **G7 (`@ts-ignore`)**: Remove the `@ts-ignore` comment and leave the empty `MapKind3` interface (empty interfaces are valid TypeScript). Or remove `MapKind3` entirely if there's no future plan. This is a 1-line fix.
