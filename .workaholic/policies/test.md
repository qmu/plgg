...
The `scripts/coverage-plgg.sh` script runs `npm run coverage` in `packages/plgg` and exits non-zero if coverage thresholds are not met. The slash command `.claude/commands/add-test.md` instructs test authors to run `scripts/coverage-plgg.sh` and maintain 90% coverage (described there as C1 and C2) when adding or improving tests.
...
Each package exposes three test-related npm scripts: `test` (one-shot compilation check plus test run), `test:watch` (Vitest interactive watch mode), and `coverage` (one-shot run with V8 coverage). Shell wrappers at the repository root invoke each package's npm script: `scripts/test-plgg.sh`, `scripts/test-plgg-foundry.sh`, `scripts/test-plgg-kit.sh`, `scripts/test-watch-plgg.sh`, `scripts/test-watch-plgg-foundry.sh`, and `scripts/test-watch-plgg-kit.sh`.
