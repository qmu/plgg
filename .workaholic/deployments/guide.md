...
- **2026-07-06 — PR #60 (`work-20260704-130317`), merge `416da301`.** Ships the 20-ticket plggpress/plggmatic product roadmap plus the new plggmatic/plggpress guide sections. Pre-merge readiness: `check-all` green on the clean CI runner for the merge commit (run 28763404210, 4m5s). `Deploy Guide` run 28763857791 succeeded (build + deploy). Custom domain live: `https://plgg.qmu.co.jp/` → HTTP 200 with valid TLS; new content confirmed — `/packages/plggmatic/` → 200 (`<title>plggmatic</title>`, a page that did not exist before this ship) and `/packages/plgg-content/` → 200; `https://qmu.github.io/plgg/` → 301 to the custom domain.
