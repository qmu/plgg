...
tickets: [20260711201840-poc2-reader-side-browser-agent.md, 20260712014500-poc2-japanese-grounding.md, 20260712023000-poc2-full-ja-article-corpus.md, 20260712024500-record-poc2-verdict.md, 20260712030000-poc3-writer-voice-assistant.md, 20260712040000-plgg-kit-realtime-ga-migration.md]
---
...
2. PoC 3 writer voice assistant: ephemeral-key mint seam, WebRTC agent loop with agent-driven `search_docs` tool calling, visible keyword-variation trail
...
- Recorded the PoC 2 verdict as proven on the portal; PoC 3 advanced to building status pending live voice judging
- Full `check-all` green twice on fresh rebuilds; plgg-kit 18 and plgg-cms 505 specs green with coverage gates intact
...
### Live voice loop only judgeable in a real browser

- **Severity:** moderate
- **Description:** PoC 3's offline bar covers the pure reducer, event decoder, and tool executor (17 specs green) and the headless WebSocket run proved the agent-driven loop, but the voice path itself (mic capture, WebRTC audio, Whisper writer-side transcription) can only be validated live. The writer-side input-transcription event name could not be probed without real audio — if writer lines are missing from the transcript during judging, that event name is the first suspect.
- **How to Fix:** Complete live acceptance judging at plgg-poc3.qmu.dev; record the verdict on the portal as PoC 2 did.
...
### PoC 3's local mintGrant duplicates the fixed plgg-kit minter

- **Severity:** low
- **Description:** PoC 3 worked around the stale plgg-kit minter with a direct GA fetch in its entrypoint before plgg-kit was fixed on the same branch; the duplication is now benign but real.
- **How to Fix:** Retire `mintGrant` onto the fixed plgg-kit minter during post-PoC integration.
...
- One open product-evaluation state, not a code-safety issue: PoC 3's live VOICE loop is not yet developer-judged (portal honestly shows `building`, verdict pending).

**Post-release instructions:**

1. Developer-judge PoC 3's live voice loop in a real browser with a microphone at plgg-poc3.qmu.dev, then record the verdict on the PoC portal (mirroring PoC 2's verdict entry).
