...
- Re-run `./scripts/check-all.sh` (the green stamp must match HEAD) and
  `cd packages/guide && npm run build` (dead-link checker clean).

### 8-3. Post-release Instructions

- None — the voice surface is dev-only and absent from `build` output; no runtime
  configuration changes ship with this branch.
