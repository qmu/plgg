...
- **When:** 2026-07-03T04:06:45+09:00
- **Target:** plgg guide (GitHub Pages, plgg.qmu.co.jp)
- **Method:** api-probe (deploy-on-merge: pre-merge readiness)
- **Status:** pass
- **Observed:** Pre-merge readiness: branch changes one workflow file + deployment metadata (no package code); mechanical dependency-order check over the new build list passes; local guide build at root base green (33 pages, 0 old-base links); DNS verified (dig plgg.qmu.co.jp CNAME = qmu.github.io.); Pages cname set, cert provisioning. Post-merge: Deploy Guide run green, site 200+TLS, 301 from old URL, https_enforced re-enabled.
