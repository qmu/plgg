...
- [ ] Typecheck comes off the per-package test hot loop: one whole-repo gate replaces 38 cold `tsc --noEmit` programs, a deliberate type error in any package still fails check-all, and the speedup is measured (#20260720123008-whole-repo-typecheck-gate.md)
- [ ] The default run is lean — no coverage instrumentation and no spawned gate process — while `--coverage` still folds the gate in-process and still fails a package below its threshold (#20260720123009-lean-default-run-inprocess-coverage.md)
