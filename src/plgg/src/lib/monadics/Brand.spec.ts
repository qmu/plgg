...
const createEmailAddress = (email: string): EmailAddress => email as EmailAddress;
...
test("Brand creates nominal type for strings", () => {
  ...
  const processUserId = (id: UserId): string => id;
...
test("Brand creates nominal type for numbers", () => {
  ...
  // But TypeScript treats them as different types
  const processProductId = (id: ProductId): number => id;
  expect(processProductId(productId)).toBe(42);
});

test("Different brand types are distinct", () => {
  const userId = createUserId("123");
  const emailAddress = createEmailAddress("user@example.com");
  ...
  const processUserId = (id: UserId): string => `User: ${id}`;
  const processEmail = (email: EmailAddress): string => `Email: ${email}`;
  
  expect(processUserId(userId)).toBe("User: 123");
  expect(processEmail(emailAddress)).toBe("Email: user@example.com");
});
...
test("Brand preserves underlying type operations", () => {
  ...
  expect(userId1.toUpperCase() as UserId).toBe("USER123");
  expect((userId1 + userId2) as UserId).toBe("user123user456");
  
  const productId1 = createProductId(10);
  const productId2 = createProductId(20);
  
  // Number operations work
  expect((productId1 + productId2) as ProductId).toBe(30);
...
test("Brand with complex underlying types", () => {
  ...
  type ConfigData = Brand<{ [key: string]: any }, "ConfigData">;
  
  const createUserData = (data: { id: string; name: string }): UserData => data as UserData;
  const createConfigData = (data: { [key: string]: any }): ConfigData => data as ConfigData;
  
  const userData = createUserData({ id: "123", name: "John" });
  const configData = createConfigData({ debug: true, port: 3000 });
  ...
  expect(Object.keys(userData)).toEqual(["id", "name"]);
  expect(Object.keys(configData)).toEqual(["debug", "port"]);
});
...
test("Brand with array types", () => {
  ...
  const createTagList = (tags: string[]): TagList => tags as TagList;
  const createScoreList = (scores: number[]): ScoreList => scores as ScoreList;
  
  const tags = createTagList(["typescript", "functional", "programming"]);
  ...
  // Array methods work
  expect(tags.join(", ")).toBe("typescript, functional, programming");
  expect(scores.reduce((a, b) => a + b, 0)).toBe(274);
});

test("Brand with primitive wrapper types", () => {
  type SafeNumber = Brand<number, "SafeNumber">;
  type SafeBoolean = Brand<boolean, "SafeBoolean">;
  
  const createSafeNumber = (n: number): SafeNumber => n as SafeNumber;
  const createSafeBoolean = (b: boolean): SafeBoolean => b as SafeBoolean;
...
test("Brand type can be null or undefined", () => {
  type OptionalUserId = Brand<string | null | undefined, "OptionalUserId">;
  
  const createOptionalUserId = (id: string | null | undefined): OptionalUserId => 
    id as OptionalUserId;
  
  const validId = createOptionalUserId("user123");
  const nullId = createOptionalUserId(null);
  const undefinedId = createOptionalUserId(undefined);
