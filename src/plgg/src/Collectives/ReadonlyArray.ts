 ...
 */
export type ReadonlyArr<T = unknown> = ReadonlyArray<T>;

/**
 * Type guard to check if a value is a ReadonlyArray.
 */
const is = (value: unknown): value is ReadonlyArr =>
  Array.isArray(value);
 ...
 */
export const { is: isReadonlyArray } = readonlyArrayRefinable;
 ...
 */
export const asReadonlyArray = <T>(
  elementCast: (value: unknown) => Result<T, InvalidError>,
) => (
  value: unknown,
  ...
  for (let i = 0; i < value.length; i++) {
    const result = elementCast(value[i]);
    if (isOk(result)) {
