...
describe("JsonReady", () => {
  describe("Atomic types round-trip", () => {
    ...
    it("should handle boolean values", () => {
      ...
      const jsonReadyFalse = toJsonReady(originalFalse);
      const restoredFalse = fromJsonReady(jsonReadyFalse);
  ...
  describe("Object types round-trip", () => {
    ...
    it("should handle objects with BigInt values", () => {
      ...
      const restored = fromJsonReady(jsonReady);
      expect(restored).toEqual(original);
      expect(typeof (restored as any).id).toBe("bigint");
      expect(typeof (restored as any).balance).toBe("bigint");
    });
    ...
    it("should handle deeply nested objects with mixed types", () => {
      ...
      const restored = fromJsonReady(jsonReady);
      expect(restored).toEqual(original);
      expect(typeof (restored as any).level1.level2.level3.bigint).toBe("bigint");
    });
  });
  ...
  describe("JSON string serialization", () => {
    ...
    it("should serialize and deserialize complex objects", () => {
      ...
      expect(typeof restoredAny.metadata.timestamp).toBe("bigint");
      expect(typeof restoredAny.summary.total).toBe("bigint");
    });
  ...
  describe("Edge cases", () => {
    ...
    it("should handle negative numbers", () => {
      ...
      expect(fromJson(toJson(-123.456))).toBe(-123.456);
    });

    it("should handle negative BigInt", () => {
      const negativeBigInt = -999999999999999999n;
      expect(fromJson(toJson(negativeBigInt))).toBe(negativeBigInt);
    });
  ...
  describe("Type assertions", () => {
    it("should maintain proper types after round-trip", () => {
      const testCases: JsonSerializable[] = [
        "string",
        42,
        true,
        false,
        ...
        { key: "value" },
        { nested: { deep: true } },
        { mixed: { str: "test", num: 1, bool: false, big: 456n } },
      ];

      testCases.forEach((testCase) => {
        const restored = fromJson(toJson(testCase));
        expect(restored).toEqual(testCase);
        expect(typeof restored).toBe(typeof testCase);
        
        // Special check for objects
        if (typeof testCase === "object" && testCase !== null) {
          expect(Object.keys(restored)).toEqual(Object.keys(testCase));
        }
      });
    });
  });
