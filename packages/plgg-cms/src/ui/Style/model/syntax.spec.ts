...
import { isHexColor } from "plgg-cms/ui/Style/model/hexColor";
import { schemes } from "plgg-cms/ui/Style/model/scheme";
import {
  type SyntaxKind,
  syntaxKinds,
  syntaxHex,
...
test("identifier and plain are deliberately absent (they inherit the default ink)", () => {
  ...
  const names: ReadonlyArray<string> = syntaxKinds.map(
    (k): string => k,
  );
  return all([
    check(names.includes("identifier"), toBe(false)),
