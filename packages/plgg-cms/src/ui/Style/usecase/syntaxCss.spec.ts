...
import { syntaxKinds } from "plgg-cms/ui/Style/model/syntax";
import { syntaxCss as syntaxCssFor } from "plgg-cms/ui/Style/usecase/syntaxCss";
import { defaultTheme } from "plgg-cms/ui/Style/model/theme";
...
test("declares a --pm-code-* property for every kind, in both schemes", () =>
  all([
    check(css, toContain(":root{--pm-code-keyword:")),
    check(css, toContain("html.dark{--pm-code-keyword:")),
...
test("maps each tok-<kind> class to its property; comment stays italic", () =>
  all([
    check(
      css,
      toContain(".tok-keyword{color:var(--pm-code-keyword)}"),
    ),
    check(
      css,
      toContain(
