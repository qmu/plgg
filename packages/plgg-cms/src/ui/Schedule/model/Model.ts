...
import { type Action } from "plgg-cms/ui/Declare/model/Action";
 ...
 * `query` text, the per-collection load `slots`, and the
 * `pending` confirmation. "Which single screen is
 * focused" is derived from `path.length`, never stored.
 *
