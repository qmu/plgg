...
/**
 * Demo 1's record collections live here, in module scope,
 * on purpose.
 *
 * WHY not the app Model: plggmatic's scheduler reads each
 * collection's `sync(() => rows)` source through a static
 * thunk fixed when the declaration is built (see
 * `plggmatic/Schedule/usecase/update.ts` `readInto`) — the
 * thunk receives only a `path`, never the app Model. A
 * record created at runtime must therefore live where that
 * thunk can still read it. Isolating the mutable arrays and
 * id counters here — one `const` cell mutated through named
 * functions, resettable — keeps that scheduler-forced seam
 * a single documented place instead of module-global `let`s
 * reassigned deep inside `update()`.
 *
 * A truly pure `update()` (records in the Model) would need
 * plggmatic to support Model-driven collection sources;
 * that is deliberately out of scope here and tracked as a
 * follow-up ticket.
 */
...
const seedClients: ReadonlyArray<Client> = [
  {
    id: "acme",
...
// The single mutable cell — a `const` object mutated in
// place through the functions below, so no module-scope
// `let` and one obvious place to reset.
const state: {
...
export const allProjects =
  (): ReadonlyArray<Project> => state.projects;

export const allClients =
  (): ReadonlyArray<Client> => state.clients;
...
// Bump and return the id counter for a section.
export const nextCount = (
  ...
  switch (section) {
    ...
    case "projects":
      state.projectCount = state.projectCount + 1;
      return state.projectCount;
  }
};

// Restore seeds and counters. Not wired into the frozen
// behavioral spec (which passes without it), but the seam
// new tests use for isolation between runs.
