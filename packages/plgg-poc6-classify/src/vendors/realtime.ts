 ...
 * PoC 3/4/4b/5): open an OpenAI Realtime session over WebRTC
 ...
 * Realtime endpoint. In PoC 6 the voice path is a BONUS —
 * the deterministic query-command path (command.ts) drives
 * every variant query without a model; when a key is present,
 * the assistant emits the SAME queries as tool calls.
