...
/**
 * The atomic, class-based styling builder — the counterpart to inline `style_`
 * that *can* express interaction states. Each part is a literal class hook
 * (`string`), a group of {@link Styles} (atoms at the base selector), or a
 * {@link Variant} (atoms under `:hover`/etc). Every declaration becomes one
 * content-hashed atomic class; the element's `class` is the joined set and the
 * rules are carried for `collectCss` to emit. `css()` is the sole authority for
 * the element's `class` (so pass hooks as string parts rather than combining
 * with `class_`).
 */
export const css = (
  ...parts: ReadonlyArray<
