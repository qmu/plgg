 ...
 * versioned HERE. `plgg-ui` ships the value-identical but
 * brand-neutral {@link defaultTheme}; `plggmatic` is its
 * brand home. This is the design-system substance the
 * extracted `plggmatic` package carries — the answer to the
 * "empty shell" risk (trip `plggmatic-extraction-cut`,
 * ticket A3): the package is a real, versionable owner of
 * the `Theme` contract and the palette-override API, not a
 * bare re-export label.
 */
 ...
 * storageKey. The one set of palette bytes lives in
 * `plgg-ui`'s `defaultTheme`; a brand override rebuilds only
 * the `palette` field — no duplication, and the
 * `plggpress → plgg-ui`-only invariant is untouched
