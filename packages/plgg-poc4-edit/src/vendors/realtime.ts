/**
 * The one genuinely browser-bound seam (PoC 3's, plus
 * the typed-text turn): open an OpenAI Realtime session
 * over WebRTC from a SHORT-LIVED ephemeral key, pump
 * every data-channel event to subscribers (the app
 * decodes them with the pure `eventOf`), send tool
 * outputs back, and send a typed writer turn over the
 * SAME data channel — the module-level `live` state here
 * is exactly why the session shell must never reload
 * (the iframe confines the dev server's
 * location.reload() to the doc pane). Only bytes move
 * here — no domain logic. Coverage-exempt PoC seam: it
 * can only run against a live browser + mic + Realtime
 * endpoint.
 */
import {
  type SoftStr,
...
/**
 * The typed-text path (new over PoC 3): a writer turn
 * as an `input_text` conversation item + a
 * `response.create`, over the SAME live data channel —
 * no second mint, no second session.
 */
