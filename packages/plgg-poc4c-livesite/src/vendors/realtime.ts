 ...
 * PoC 3/4): open an OpenAI Realtime session over WebRTC
 ...
 * Realtime endpoint. (vendor-neutrality: the vendor lives
 * behind this thin adapter; the domain speaks in our own
 * types.)
 */
import {
  type SoftStr,
...
import { REALTIME_MODEL } from "../poc4b.ts";
...
/**
 * The typed-text path: a writer turn as an `input_text`
 * conversation item + a `response.create`, over the SAME
 * live data channel — no second mint, no second session.
 */
