...
- **[`packages/plgg-cms/`](packages/plgg-cms/)** - The dynamic content-management surface that pairs with plggpress: CMS-owned content indexing/query, admin UI, durable-domain derivation, a read-only content delivery API, OIDC auth, content editing, media, stakeholder submission, ops, the content MCP tools, and agent surfaces — composed onto plggpress's `framework` seam and served as an always-on `node:http` instance (the `plgg-cms serve` bin, D5's dynamic half)
- **[`packages/plgg-mcp/`](packages/plgg-mcp/)** - The MCP protocol substrate: typed tool registry + dispatch (`initialize`, `tools/list`, `tools/call`), JSON-RPC 2.0 framing, transport-agnostic frame handling, and a stdio server seam — no content store, no default tool set, `node:sqlite` pinned absent from its module graph; plgg-cms composes its content tools onto it
- **[`packages/plgg-search/`](packages/plgg-search/)** - The browser-shippable full-text search core (PoC 1's proven artifact): one shared latin+CJK tokenizer (`Intl.Segmenter` / bigram, the strategy carried inside the index so build and query cannot mismatch), a heading-scoped markdown chunker, an inverted-index builder, and from-scratch BM25 — zero node builtins, pinned by spec; the SSG build step and the reader's browser run the same code
